$(function() {
  ...
  $('.movie-button').on('click',function(){
    var my_api_key = 'aa0233e4fa8a52a510dc5cbdfea68379';
    var title = $('.movie').val();
    if (title=="") {
...
function load_details(my_api_key,title){
  if (!title || title.trim() === "") {
    ...
    console.log("Sending request for movie:", title);
  ...
  $.ajax({
    ...
    success: function(movie){
      console.log("Response from tmdb api:", movie);  // Log API response in Console

      if(movie.results.length<1){
        console.error("No movie found for:", title);
        $('.fail').css('display','block');
        $('.results').css('display','none');
     ...
     error: function(xhr, status, error) {
            ...
            alert("Invalid Request: API call failed." +xhr.responseText);
            // $("#loader").delay(500).fadeOut();
    },
  });
}
